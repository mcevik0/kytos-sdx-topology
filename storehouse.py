...
class StoreHouse:
    ...
    def create_box(self):
        ...
        content = {
            "namespace": "kytos.sdx.storehouse.version",
            "callback": self._create_box_callback,
            "data": {"version": self.counter, "oxp_name": "", "oxp_url": ""},
        }
    ...
    def _create_box_callback(self, _event, data, error):
        """Execute callback to log the output of the create_box function."""
        if error:
    ...
    def update_box(self):
        """Update existing box with a new version value after topology change\
                is detected."""
        self._lock.acquire()  # pylint: disable=R1732
    ...
    def _get_box_callback(self, _event, data, error):
        """Execute a callback to log the output of the get_stored_box"""
        self.box = data
